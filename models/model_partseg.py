...
class MLPHead(nn.Module):
    def __init__(self, args):
        ...
        self.nn = nn.Sequential(nn.Conv1d(emb_dims + 64, emb_dims // 2, 1, bias=False),
    ...
    def forward(self, *input):
        ...
        # (batch_size, emb_dims, num_points)
        # weights = input[2].transpose(1, 2)

        N = attn.size(2)
        # B x emb_dims x N  (emb_dims * 2 total)
        # weights_max = weights.max(dim=2, keepdim=True)[0].repeat(1, 1, N)
        # weights_mean = weights.mean(dim=2, keepdim=True).repeat(1, 1, N)
        # (batch_size, num_categories, 1)
        lbl = lbl.unsqueeze(-1)
        # (batch_size, num_categories, 1) -> (batch_size, 64, 1)
        lbl = self.label_conv(lbl)
        # B x 64 x N
        lbl = lbl.repeat(1, 1, N)
        # (batch_size, emb_dim + 64, 2048)
        x = torch.cat((lbl, attn), dim=1) 
        return self.nn(x)
...
class Net(nn.Module):
    def __init__(self, args):
        ...
        self.transformer = nn.Transformer(d_model=args.emb_dim, nhead=args.n_heads,
                                        num_encoder_layers=args.n_blocks, num_decoder_layers=args.n_blocks,
                                        dim_feedforward=args.ff_dims, dropout=args.dropout,
                                        activation=nn.LeakyReLU(negative_slope=0.2), batch_first=True)
        self.attention = nn.MultiheadAttention(embed_dim=args.emb_dim, num_heads=args.n_heads, dropout=args.dropout, batch_first=True)
    ...
    def forward(self, src, lbl):
        ...
        src_embedding_p = self.transformer(src_embedding, tgt_embedding)
        tgt_embedding_p = self.transformer(tgt_embedding, src_embedding)
        # (batch_size, num_points, emb_dims)
        scores, _ = self.attention(
            query=tgt_embedding_p, key=src_embedding_p, value=src_embedding_p, need_weights=False)
